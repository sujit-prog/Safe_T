...
async function main() {
  ...
  // Let's seed across a large area: lat 20.0 to 22.0, lng 83.0 to 86.5
  ...
  // Add a very high density of incidents specifically around Burla to Sambalpur route (Lat ~21.5, Lng ~83.9) 
  // so the user immediately sees a RED zone.
  for(let i = 0; i < 50; i++) {
    await prisma.incidentReport.create({
      data: {
        type: "Accident",
